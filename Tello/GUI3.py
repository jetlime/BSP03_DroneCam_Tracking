...
# Trained opencv model that is able to recognize faces
face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')



# Initalising variables needed for the drone
# Follow a person or not 
me = Tello()
me.connect()
followMode = False
...
me.yaw_velocity = 0
me.speed = 0
# Init the direction the drone needs to go to follow a face
dir = 0 
...
# Create an instance inside the root to display information of the drone
frame = Label(root, text="battery is " + me.get_battery() + "percent")

# change the color of the progress bar in function of the drones battery level
if int(me.get_battery()) > 50 :
    color = "green"
elif int(me.get_battery()) < 50 and int(me.get_battery()) > 20:
    color = "orange"
else :
    color = "red"
'''
# Create a progress bar
batteryLevel = tkinter.ttk.Progressbar(frame, orient = HORIZONTAL, length = 300)
batteryLevel.place(x = 0, y = 200)
batteryLevel.pack()
batteryLevel.config(mode = "determinate", maximum = 100, value = me.get_battery())

barometer = Label(frame, text = "Atmospheric pressure is " + me.get_barometer())
barometer.place(x = 0, y = 400)

flightTime = Label(frame, text = 'Flight Time : ' + me.get_flight_time())
flightTime.place(x = 0, y = 600)
'''
def stream() :
...
but14=Button(l,padx=5,pady=5,width=14,bg='red',fg='black',relief=GROOVE,command=followMode,text="Follow mode",font=('helvetica 15 bold'))
