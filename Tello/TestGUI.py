...
def distance(face1,x,y,w,h) :
    center1 = (int(face1[0]+(face1[2]/2)),int(face1[1]+(face1[3]/2)))
    center2 = (int(x+(w/2)),int(y+(h/2)))
    a = numpy.array(center1)
    b = numpy.array(center2)
        ...
        if timing :
            ...
            if followmode :
                ...
                for (x,y,w,h) in facesTuples :
                    ...
                    if distance(nearestFace,bbox[0],bbox[1],bbox[2],bbox[3]) > dist :
                        nearestFace = (x,y,w,h)
